...
function minWindow(s: string, t: string): string {
    ...
    const sIndexes = new Map<string, number[]>();
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        if (tCountVal) {
            let sIndexesVal = sIndexes.get(s[i]) ?? [];
            sIndexesVal.push(i);
            sIndexesArr.push(i);
            if (sIndexesVal.length <= tCountVal) {
                count++;
            } else {
                sIndexesArr.splice(sIndexesArr.indexOf(sIndexesVal[0]), 1);
                sIndexesVal.splice(0, 1);
            }
